...
async function clientFor(document: vscode.TextDocument): Promise<HostClient> {
  const folder = folderFor(document); const projectPath = await discoverProject(folder);
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  context.subscriptions.push(vscode.languages.registerCompletionItemProvider({ language: 'segusum' }, { provideCompletionItems: async (document, pos) => { try { const client = await clientFor(document); const result = await client.request('completion', { path: document.uri.fsPath, line: pos.line + 1, column: pos.character + 1, text: document.getText() }); return (result ?? []).map((item: any) => new vscode.CompletionItem(item.label)); } catch (e) { log(`Completion failed: ${e}`); return []; } } }, '.'));
  ...
  const watcher = vscode.workspace.createFileSystemWatcher('**/*.{seg,cs}'); watcher.onDidChange(uri => { log(`File changed ${uri.fsPath}; invalidating clients.`); for (const client of clients.values()) client.invalidate(); }); watcher.onDidCreate(uri => { log(`File created ${uri.fsPath}; invalidating clients.`); for (const client of clients.values()) client.invalidate(); }); watcher.onDidDelete(uri => { log(`File deleted ${uri.fsPath}; invalidating clients.`); for (const client of clients.values()) client.invalidate(); }); context.subscriptions.push(watcher);
...
export function deactivate() { for (const client of clients.values()) client.dispose(); requestCts?.dispose(); }
