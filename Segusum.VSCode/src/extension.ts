...
class HostClient {
  ...
  request(method: string, params: any, token?: vscode.CancellationToken): Promise<any> { const id = this.next++; return new Promise((resolve,reject) => { this.pending.set(id,{resolve,reject}); this.child?.stdin.write(JSON.stringify({id,method,params})+'\n'); if (token) token.onCancellationRequested(() => this.cancel(id)); }); }
  cancel(id: number): void { this.child?.stdin.write(JSON.stringify({id: this.next++, method:'cancel', params:{requestId:id}})+'\n'); }
  invalidate(): void { void this.request('invalidate', {}); }
...
export async function activate(context:vscode.ExtensionContext){
  const root=vscode.workspace.workspaceFolders?.[0]?.uri.fsPath; if(!root) return; client=new HostClient(); try{await client.start(root);}catch(e){vscode.window.showErrorMessage(String(e)); return;}
  ...
  context.subscriptions.push(vscode.languages.registerCompletionItemProvider({language:'segusum'}, {provideCompletionItems:async(d,p)=>{const x=await call('completion',{path:d.uri.fsPath,line:p.line+1,column:p.character+1}); return (x??[]).map((a:any)=>new vscode.CompletionItem(a.label));}}, '.'));
