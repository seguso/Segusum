...
$frozen = Join-Path $litgirRoot 'docs\migration-inputs\worldBeforeRoomChange.cs'
$runtimeRoot = Join-Path $litgirRoot 'WebApiLitGir'
$runtimeSeg = Join-Path $runtimeRoot 'Gameplay\BeforeRoomChanged.seg'
$tempRoot = Join-Path $env:TEMP 'segusum-litgir-beforeroomchange'
$tempSeg = Join-Path $tempRoot 'BeforeRoomChanged.generated.seg'
$cliProject = Join-Path $segusumRoot 'Segusum.Migration.Cli\Segusum.Migration.Cli.csproj'

if (-not (Test-Path -LiteralPath $frozen)) { throw "Frozen BeforeRoomChanged source not found: $frozen" }
...
Write-Host "Generating BeforeRoomChanged SEG from frozen source: $frozen"
Invoke-Cli @(
...
if (-not (Test-Path -LiteralPath $tempSeg)) { throw "Generated temporary SEG file not found: $tempSeg" }

$manual = @(Select-String -LiteralPath $tempSeg -Pattern 'C2SEG-MANUAL' -SimpleMatch)
if ($manual.Count -ne 0) { throw "Generated BeforeRoomChanged contains $($manual.Count) C2SEG-MANUAL markers." }
